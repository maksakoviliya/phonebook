  ...
  <div class="mr-auto ml-4 w-50">
    <form action="#" class="search-form">
      <div class="form-group">
        <label class="bmd-label-floating d-flex align-items-center"><i class="material-icons">search</i> Поиск по контактам</label>
      ...
      <div class="search-results" v-if="search">
        <ul class="list-unstyled mb-0" v-if="results.length > 0">
          <li v-for="result in results" :key="result.id">
            <a :href="path+result.id+'/edit'">{{ result.last_name }} {{ result.first_name }}</a>
          </li>
        </ul>
        <div v-else>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.path = this.baseUrl + '/contacts/'
    },
    methods: {
        fetch() {
            axios.get('/api/search/contacts', { params: { search: this.search, phonebookId: this.phonebookId } })
                .then((response) => {
